...
const PREC = {
    COMMENT: 0,          // //  /*  */
    ASSIGN: 1,           // =  += -=  *=  /=  %=  &=  ^=  |=  <<=  >>=  >>>=
    DECL: 2,             // var x; or type declarations
    ELEMENT_VAL: 2,      // For example, array or list element access
    TERNARY: 3,          // ?:
    OR: 4,               // ||
    AND: 5,              // &&
    BIT_OR: 6,           // |
    ...
    BIT_AND: 8,          // &
    EQUALITY: 9,         // ==  !=
    GENERIC: 10,         // For generics, e.g., List<T> or other generic types
    REL: 10,             // <  <=  >  >=  instanceof
    SHIFT: 11,           // <<  >>  >>>
    ADD: 12,             // +  -
    MULT: 13,            // *  /  %
    CAST: 14,            // (Type)
    OBJ_INST: 14,        // new (object instantiation)
    UNARY: 15,           // ++a  --a  a++  a--  +  -  !  ~
    ARRAY: 16,           // [Index] for array or list access
    OBJ_ACCESS: 16,      // . (field or method access)
    PARENS: 16,          // (Expression) for grouping expressions
    CLASS_LITERAL: 17,   // Class literals, e.g., SomeClass.class
  };
...
module.exports = grammar({
  ...
  extras: $ => [
    $.line_comment, // Comments that start with '//'
    $.block_comment, // Comments that are more of like multiline '*/  */'
    /\s/,
  ],

  supertypes: $ => [
    $.expression,              // Expression like 'x + y'
    $.declaration,             // Declaration of variables or constants
    $.statement,               // A line of executable code like an assignment or method call
    $.primary_expression,      // Primary elements like literals, variables, method calls
    $._literal,                // LITERAL values like '5', 'true', '"hello"'
    $._type,                   // Types such as 'int', 'string', etc.
    $._simple_type,            // Basic types like 'int', 'boolean'
    $._unannotated_type,       // Types without annotations, for example 'int' vs 'Nullable<int>'
    $.module_directive,        // 'import', 'package' statements or directives
  ]
  ,

  inline: $ => [
    $._name,                    // This refers to an element that represents as identifier or variable name
    $._simple_type,             // This rule refers to a simple type in the language, which typically represents basic data types like integers, strings, or other simple types.
    $._class_body_declaration,  // This rule represents a class body declaration in Gosu. A class body declaration refers to the definitions within a class, such as methods, fields, constructors, etc.
    $._variable_initializer,    // This rule represents the initialization of a variable in Gosu. A variable initializer is the expression or value assigned to a variable when it is declared.
  ]
  ,
